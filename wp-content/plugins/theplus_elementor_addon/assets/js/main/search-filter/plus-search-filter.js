					...
					form.forEach(function(self) {
						let seafield = JSON.parse(self.dataset.field);
							...
							if(seafield.checkBox){
								...
								if(GetHtml.length > 0){
									GetHtml.forEach(function(item,idx){
										let Name = (seafield.checkBox[idx] && seafield.checkBox[idx].name) ? seafield.checkBox[idx].name : '',
											Geturl = (Name) ? params.get('ck_'+Name+'_'+idx) : '';
											if(Geturl){
												Geturl.split(",").forEach(function(item1,idx1){
													GetHtml[idx].querySelector('#tp-'+item1).checked = 1;	
												})
							...
							if(seafield.radio){
								...
								let GetHtml1 = $scope[0].querySelectorAll('.tp-wp-radio');
								if(GetHtml1.length > 0){
									GetHtml1.forEach(function(item,idx){
										let Name = (seafield.radio[idx] && seafield.radio[idx].name) ? seafield.radio[idx].name : '',
											...
											if(Geturl){
												Geturl.split(",").forEach(function(item1,idx1){
													GetHtml1[idx].querySelector('#tp-'+item1).checked = 1;
												})
												fieldValue = radioHandler(seafield.radio, rotag)
											}
									});
							...
							let dateTag = [];
							if(seafield.date){
								let GetHtml = self.querySelectorAll('.tp-date-wrap');
								if(GetHtml.length > 0){
							...
							if(tagHandle){
								tagHandle.innerHTML = inputTag.join(' ') + alphaTag.join(' ') + checkList.join(' ') + " " + rotag.join(' ') + " " + seleTag.join(' ') + " " + dateTag.join('') + " "+ colorTag.join('') + " "+ imageTag.join('') + " "+ buttonTag.join('') + " "+ tabbingTag.join(''); 
							}
            ...
            check.forEach(function(field,index){
                ...
                if(Name){
                    ...
                    if(chidden) {
						...
						if(basic.URLParameter){
							let Urldata = fieldValue[Name]['value'].toString();
								urlHandler('ck_'+Name+'_'+index, Urldata)
						}
                    }
                }
            })
			...
			date.map(function(datefield){
				let Name = (datefield.name) ? datefield.name : '',
					...
					if(datesele.length > 0){
						let start = (datesele[0].value) ? datesele[0].value : '',
							last = (datesele[1].value) ? datesele[1].value : '',
							Title='';
							...
							tagList[i].addEventListener('click', function(e) {
								...
								var close = this,
									...
									if(type === 'checkBox'){
										...
										let GetHtml = $scope[0].querySelectorAll('.tp-wp-radio');
											GetHtml.forEach(function(item){
												item.querySelector('#tp-'+Id).checked = 0;
											});
											fieldValue[key]['value'] = '';
