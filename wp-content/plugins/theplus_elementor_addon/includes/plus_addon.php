...
function theplus_filter_post(){
	...
	foreach ($optiondata as $key => $postdata) {
		...
		if($post_type == 'post'){
			...
			if(!empty($texonomy_category) && !empty($category)){
				$args['tax_query'] = array(
					array(
		...
		if (!empty($postTag) && $post_type == 'post') {
		...
		if ($FilterType == 'search_list'){
			...
			if(!empty($postdata['seapara'])){
				...
				if($post_type == 'post'){
					$tags_args=[];
					$category_args='';
					// $tags_args=$category_args=$attr_tax=$woo_attrQ=[];
				}else if($post_type == 'product'){
					$tags_args=$category_args=$attr_tax=$woo_attrQ='';
				}
					...
					if($TypeValue == 'taxonomy'){
						if($FieldValue == 'rating' && $post_type == 'product' && !empty($DataValue)){
							...
							if(!empty($Generic['GFEnable'])){
								$Result = ($Generic['GFSType'] == 'fullMatch') ? $DataValue.'%' : '%'.$DataValue.'%';

								$PPublish = "AND {$wpdb->posts}.post_status = 'publish' ";
								$PType = $wpdb->prepare( "AND {$wpdb->posts}.post_type = %s", $post_type );
								if($Generic['GFTitle']){
									$PTitle = $wpdb->prepare( "{$wpdb->posts}.post_title LIKE %s ", $wpdb->esc_like($Result) );
								...
								if($Generic['GFContent']){
									$Pcontent = $wpdb->prepare( "{$wpdb->posts}.post_content LIKE %s ", $wpdb->esc_like($Result) );
								...
								if($Generic['GFExcerpt']){
									$Pexcerpt = $wpdb->prepare( "{$wpdb->posts}.post_excerpt LIKE %s ", $wpdb->esc_like($Result) );
								...
								if($Generic['GFName']){
									$Pname = $wpdb->prepare( "{$wpdb->posts}.post_name LIKE %s ", $wpdb->esc_like($Result));
						...
						}else if(($FieldValue == 'color' || $FieldValue == 'image' || $FieldValue == 'button') && $post_type == 'product' && !empty($DataValue)){
							$woo_attrQ = array(
								'taxonomy' => $NameValue,
								'field' => 'id',
								'terms' => $DataValue,
					...
					if(!empty($keyEnable)){
						if($post_type == 'post'){
							if($NameValue == 'category' && !empty($DataValue)){
								...
								if( !empty($texonomy_category) && !empty($NameValue) ){
									$tags_args = array(
										array(
											'taxonomy'=>$NameValue,
											'field'=>'term_id',
							...
							if($NameValue == 'product_cat' && !empty($DataValue)){
								$category_args = array(
									'taxonomy' => $NameValue,
									'field'    => 'id',
									'terms'    => $DataValue
								);
							}else if($NameValue == 'product_tag' && !empty($DataValue)){
								$tags_args = array(
									'taxonomy' => $NameValue,
									'field'    => 'id',
									'terms'    => $DataValue
				...
				if( !empty($category_args) || !empty($tags_args) || !empty($attr_tax)|| !empty($woo_attrQ) ){
					$args['tax_query'] = [ 'relation' => 'AND', $category_args, $tags_args, $attr_tax, $woo_attrQ ];
				}
				if(!empty($TmpPostID)){
...
function tp_search_bar(){
	...
	$Def_post = (!empty($_POST['DefaultData']) && !empty($_POST['DefaultData']['Def_Post'])) ? $_POST['DefaultData']['Def_Post'] : 'any';
	...
	if(!empty($_POST['text'])){
		...
		if( !empty($ACFEnable) || (!empty($GFilter['GFEnable'] && $Def_post != 'any') )){
			$AllData=$GTitle=$GExcerpt=$Gcontent=$GName=$PCat=$PTag=$ACFData=[];

			$Result = ($GFSType == 'fullMatch') ? "{$sqlContent}%" : "%{$sqlContent}%";
			...
			if(!empty($PostType)){
				if(!empty($Enable_DefaultStxt)){
					$start = array('[', ']');
					$end = array('','');
					$PostTypeString  = str_replace($start, $end, json_encode($PostType,true) );
					$DType = " AND post_type IN (" . $PostTypeString . ")";
				}else{
